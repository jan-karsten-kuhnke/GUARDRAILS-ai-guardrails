...
export const fetchPrompt = (message: string, conversationId: string | null , isOverride: boolean , modelName: string | undefined , task: string, isPrivate : boolean) => fetch(`${import.meta.env.VITE_CHAT_SERVICE_URL}/completions`, { method: "POST", headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${AuthService.getToken()}` }, body: JSON.stringify(conversationId ? {message: message, conversation_id: conversationId , isOverride: isOverride, model_name : modelName, task : task, is_private:isPrivate }  : { message: message })});
